...
- **PostHog dashboard** — `Workshop — Dastyare Social` (see below) with the three funnel insights and a weekly email subscription.
...
The dashboard **Workshop — Dastyare Social** ([open](https://us.posthog.com/project/103916/dashboard/2041958)) is filed in the **Workshop** folder ([`folders/Workshop`](https://us.posthog.com/project/103916/dashboards)). It contains three insights:

| Insight | Type | Steps |
| --- | --- | --- |
| Registration funnel (`11416465`) | Funnel (14-day) | `landing_page_viewed` → `registration_cta_clicked` → `registration_form_continue` → `registration_form_submit_success` → `confirmation_page_viewed` |
| Landing engagement (`11416474`) | Funnel (14-day) | `$pageview` → `scroll_depth_50` → `registration_cta_clicked` |
| CTA performance by section (`11416483`) | Funnel (14-day), broken down by `cta_location` | `registration_cta_clicked` → `registration_form_submit_success` |

A weekly email subscription (`137941`, "Workshop weekly — Dastyare Social") exports all three insights to `iamomidshabab@gmail.com` every Monday (AI summary emphasising conversion and drop-off).
